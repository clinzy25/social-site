...
class Post
{
  ...
  public function submitPost($body, $user_to)
  {
    ...
    if ($check_empty != '') {
      ...
      $query = mysqli_query(
        $this->con,
        "INSERT INTO posts VALUES('', '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0')"
      );
  ...
  public function loadPostsFriends()
  {
    ...
    while ($row = mysqli_fetch_array($data)) {
      ...
      if ($row['user_to'] == 'none') {
        ...
        $user_to =
          "to <a href='" . $row['user_to'] . "'>" . $user_to_name . '</a>';
      }
    ...
    if ($interval->y >= 1) {
      // If post is more than a year old
      if ($interval == 1) {
        $time_message = $interval->y . ' year ago'; // 1 year ago
      } else {
        $time_message = $interval->y . ' years ago'; // 1+ years ago
      }
      // If post is less than a year old
    } elseif ($interval->m >= 1) {
      // See how many days old
      if ($interval->d == 0) {
        $days = ' ago';
      } elseif ($interval->d == 1) {
        $days = $interval->d . ' day ago'; // ex 1 month 1 day ago
      } else {
        $days = $interval->d . ' days ago'; // ex 1 month 3 days ago
      }
      // See how many months old
      if ($interval->m == 1) {
